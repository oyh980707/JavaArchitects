...
位图（bitmap）广泛用于资源管理，是一种管理资源的方式、手段。“资源”包括很多，比如 内存或硬盘，对于此类大容量资源的管理一般都会采用位图的方式。位是指bit，即字节中的位，1字节中有8个位。图是指map，地图本质上就是映射的意思，映射即对应关系。综合起来，位图就是用字节中的1位来映射其他单位大小的资源，按位与资源之间是一对一的对应关系。
...
程序是指静态的、存储在文件系统上的、尚未执行的指令代码。他是实际运行时程序的映像。

进程程根据线程数分为：单线程进程、多线程进程

### 线程

线程是一套机制，此机制可以为一般的代码块创造它所依赖的上下文环境，从而让代码块具有独立性，因此在原理上线程能使一段函数成为调度单元(或称为执行流)，使函数能被调度器“认可”，从而能够被调度到处理器上执行。

执行流
执行流对应于代码，大到可以是整个程序文件，即进程，小到可以是一个功能独立的代码块，即函数
执行流是独立的，它的独立性体现在每个执行流都有自己的栈、一套自己的寄存器映像和内存资源，
这是 Intel 处理器在硬件上规定的，其实这正是执行流的上下文环境。因此，我们要想构造 一 个执行流， 就要为其提供这一整套的资源。

### 进程与线程的关系、区别简述

对于处理器来说，进程是一种控制流集合，集合中至少包含一条执行流，执行流之间是相互独立的，但它 们共享进程的所有资源，它们是处理器的执行单位，或者称为调度单位，也就是线程。任务其实就是执行流，要么是大的执行流单线程的进程，要么是小的执行流线程。

利用线程提速
1. 让处理器多执行自己进程中的代码，这样进程执行完成得就快（多线程技术）。
2. 在内核级线程中可以避免阻塞整个进程。

### 进程的身份证（PCB）

操作系统为每个进程提供了一个PCB（Process Control Block），即程序控制块，它就是进
程的身份证，用它来记录与此进程相关的信息，比如进程状态、PID、优先级等。每个进程都有自己的PCB，所有PCB放到一张表格中维护，这就是进程表，调度器可以根据这张表选择上处理器运行的进程。
...
在用户空间中实现线程，操作系统根本就不会意识到线程的存在，因为操作系统调度器只会以整个进程的方式调度，将处理器的使用权交给这个进程，由进程中的调度器自己去协调分配处理器时间。用户线程要肩负起调度器的责任，因此除了要实现进程内的线程调度器外，还要自己在进程内维护线程表。

在用户进程中实现线程有以下优点：
- 可移植性强，在不支持线程的操作系统上，也可以写出完美支持线程的用户程序。
- 调度算法是由用户程序自己实现的，可以根据实现应用情况为某些线程加权调度。
- 将线程的寄存器映像装载到CPU时，可以在用户空间完成，不用陷入到内核态，这样就免去了进入内核时的入栈及出栈操作。

在用户进程中实现线程有以下优点：
- 进程中的某个线程若出现了阻塞(通常是由于系统调用造成的)，操作系统不知道进程中存在线程，它以为此进程是传统型进程(单线程进程)，因此会将整个进程挂起，即进程中的全部线程都无法运行
- 线程未在内核空间中实现，对于操作系统来说调度器的调度单元是整个进程，并不是进程中的线程，所以时钟中断只能影响进程一级的执行流。无法调度用户进程中的线程。这就导致了但凡进程中的某个线程开始在处理器上执行后，只要该线程不主动让出处理器，此进程中的其他钱程都没机会运行。

线程在用户空间实现和在内核空间实现相比，只是在内部调度时少了陷入内核的代价，确实相当于提速，但由于整个进程占据处理器的时间片是有限的，这有限的时间片还要再分给内部的线程，所以每个线程执行的时间片非常非常短暂，再加上进程内线程调度器维护线程表、运行调度算法的时间片消耗，反而抵销了内部调度带来的提速。
...
在内核空间中实现线程的优点：
- 相比在用户空间中实现线程，内核提供的线程相当于让进程多占了处理器资源（进程中的多线程）。
...
我们知道，计算机加点后，CPU就开始工作了，其实这时候已经有一条工作流了，相当于主线程。那么此时他可以创建多个进程和线程去伪并行的完成更多的事。

这里我们就看一个线程如何被创建出来的，在内核空间中实现线程，那么总会有一个主线程来创建其他的线程，那么主线程就是进程创建的时候被被创建出来的一个线程，再由这个线程去创建其他的线程。一生二、二生四...

主线程如何创建线程呢？

1. 首先主线程需要初始化一些信息，比如构建好就绪队列等一些资源，初始化完后将自己加入运行队列中进行管理，也就是初始化自己的PCB。
2. 主线程开始创建线程，先去申请一页物理内存4KB，它来表示PCB，也就是维护线程的一些信息
3. 将这个线程加入到一个就绪队列和一个维护所有的线程队列，这些队列我们暂且用双向链表来实现。这里加入是将线程的tag标识(维护在PCB中)作为节点维护在链表中，能够通过这个tag找到对应的线程即PCB
4. 线程的首次执行有些不一样，因为线程调度是通过时钟中断的方式来进行驱动的。当中断来临，CPU会自动关中断，然后进行调度，起始就是保存上下文，线程切换，调度完之后退出中断，中断打开。在这种情况下，如果一个线程被调用必须打开中断开关即开中断，才可以被时钟中断打断，进行调度其他线程执行。而打开中断这个动作放到线程首次执行去完成。总的来说，就是将线程首次执行引导到我们制定的方法中执行进行开中断操作，然后调用最终的线程方法。这样在执行线程方法时就可以被中断打断进行线程切换了。
5. 调度中有一个重要的操作，在时钟中断时，调用中断处理函数，我们在中断处理程序中实现线程的上下文切换，最后通过汇编代码ret指令实现将新的线程指令指针弹如EIP中，也就是使用ret指令就可以直接返回到这个线程调度前的位置接着执行。可以理解线程调度起始就是将他们的寄存器、返回地址等等信息保存，待下次被获得时间片，利用ret指令将返回地址写入eip寄存器，处理器接着eip指向的指令执行。
6. 线程切换完之后，退出中断，接着执行EIP寄存器指向的指令，此时EIP执行的指令已经是被切换成新的线程的指令了。
...
互斥也可称为排他，是指某一时刻公共资源只能被1个任务独享，即不允许多个任务同时出现在自己的临界区中。公共资源在任意时刻只能被一个任务访问，即只能有一个任务在自己的临界区中执行，其他任务想访问公共资源时，必须等待当前公共资源的访问者完全执行完他自己的临界区代码后(使用完资源后)再开始访问。
...
此概念是由荷兰人 E.W.Dijkstra 在 1965 年首次提出的，它是一种程序设计构造方法。其原型来自于铁路上的信号灯。
在计算机中，信号量就是个0以上的整数值，当为0时表示己无可用信号，或者说条件不再允许，因此它表示某种信号的累积“量“，故称为信号量。虽然信号量听上去和信号相似，像是某种通信机制，但其实它和通信无关，信号量是种同步机制。
信号量就是个计数器，它的计数值是自然数，用来记录所积累信号的数量。由于Dijkstra是荷兰人，他用 P、V操作来表示信号量的减、增，这两个都是荷兰语中的单词的缩写。P是指Proberen，表示减少，V是指单词Verhagen，表示增加。
...
这里可以采用二元信号实现锁机制，具体步骤如下：
1. 线程A进入临界区前先通过V操作获得锁，此时信号量的值便为0。
2. 后续线程B再进入临界区时也通过V操作获得锁，由于信号量为0，线程B便在此信号量上等待，也就是相当于线程B进入了睡眠态。
3. 当线程A从临界区出来后执行P操作释放锁，此时信号量的值重新变成1，之后线程A将线程B唤醒。
4. 线程B醒来后获得了锁，进入临界区。
...
1. 当线程去获取锁的时候，由于锁被其他线程占有，只能阻塞自己，判断条件(锁持有的信号量是否为0)
2. 将自己加入到当前锁的等待队列中去
3. 修改自己的线程状态为阻塞状态
4. 关闭中断，然后调用schedule()切换到其他线程执行
5. 等待被唤醒后，重新执行，重新获取锁
6. 当获取到锁，将锁持有的信号量-1

#### 线程的唤醒

1. 当有线程释放锁时，会将锁持有的阻塞队列中弹出一个线程，将该线程唤醒，进入唤醒操作
2. 将被唤醒的线程加入到就绪队列的头部去为了更快的执行，然后待CPU调度执行，将状态也设置为就绪状态
3. 释放锁的线程继续操作将锁持有的信号量+1
...
汇编器将汇编程序源码中的section或segment编译成节，此时只是生成目标文件，连接器将这些目标文件中属性相同的节合并成段，因此一个段由多个节组成。

按照属性来划分节（也就是段描述符中的一些权限校验等属性）
- 可读可写的数据段，例如.data和为初始化的.bss
- 只读课执行的代码，例如.text和初始化代码.init
- 只读数据，例如.rodata，一般字符串存储在此节

所有的节可以归并到以上三类，为了方便操作系统加载程序时的内存分配。由链接器把目标文件中相同属性的节归并之后的节的集合称为段(segment)，也就是C程序运行时内存空间分布的代码段、数据段等段。
