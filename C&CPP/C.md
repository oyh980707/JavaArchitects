...
在C语言中，互斥量被分为三种类型：mtx_plain、mtx_recursive、mtx_timed。

mtx_plain：最简单的互斥量，不可重入。
mtx_recursive：可重入互斥量。
mtx_timed：具有超时的属性，通过配合mtx_timedlock函数实现带超时限制的互斥量加锁。

原子操作：C11提供的stdatomic.h头文件具有原子操作的定义。
条件变量：条件变量为我们提供一种线程之间“通知”的能力。
线程本地变量：类似于java中的ThreadLocal，将值的副本存放在只能当前线程可见区域


## 信号

信号是操作系统提供的一种可以用来传递特定消息的机制。信号产生是随机的，而且是一种典型的异步事件处理方式。

信号与软件中断：中断可以分为硬件中断和软件中断，硬件中断就是计算机硬件触发的中断，软件由计算机通过int等指令引起的CPU执行流程的临时转移过程。系统调用便是其中一种，信号也是软件中断的一种。

以除零为操作背景展示一个信号的基本交互逻辑过程：
1. CPU执行除法指令idev；
2. 发现除零异常，CPU暂停当前执行的程序，并将控制权转移到操作系统；
3. 操作系统将信号SIGFPE发送给出错的程序；
4. 操作系统根据情况执行相应的信号处理程序(被注册的相应函数)；
5. 信号处理程序执行完毕后，若程序未退出，则将程序恢复到之前的中断点，即CPU会重新执行除法指令idev；

例如Linux中的kill -9命令就是给应用程序发送信号，应用程序接受到该信号就会做好退出的准备并退出，如果应用程序固执不退出，该命令也将无法杀死该进程。

## 断言

分为静态断言和运行时断言

静态断言：约束程序在编译时需要满足的一定要求。
运行时断言：可以在程序的运行过程中判断一些支持程序正常运行的假设性条件是否满足。


## 高性能C程序

1. 利用高速缓存：时间局部性和空间局部性、缓存行
2. 代码内联：inline关键字建议编译器方法内联、减少方法过程中消耗的性能
3. restrict关键字：用于指针类型，表示这个指针是访问这块内存的唯一方式
4. 消除不必要的内存引用：减少内存的访问次数，提升一定的性能
5. 循环展开：从宏观上来讲，机器指令的执行由串行变为并行流水线执行，执行效率得到提升
6. 优先使用条件传送指令：根据CPU标志位的不同状态，有条件的传送数据到某个特定位置，可以解决CPU分支预测失败带来的性能损耗例如三目运算符
7. 使用更高的编译优化等级：-O0、-O1、-O2、-O3...通过编译器对程序的优化程度
8. 尾递归调用优化：通过将函数的递归调用优化为循环结构减少call指令的调用次数，减少栈帧的创建与销毁过程，提升程序的执行性能
